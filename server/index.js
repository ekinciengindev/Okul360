...
app.post('/api/pickups', async (req, res, next) => {
  try {
    ...
    const newPickup = await db.PickupRequest.create({
      ...
      code,
      latitude: latitude ? Number(latitude) : 41.0525,
      longitude: longitude ? Number(longitude) : 28.6895,
      schoolId
    });
...
app.put('/api/pickups/:id', async (req, res, next) => {
  try {
    ...
    if (pk) {
      ...
      res.status(404).json({ message: 'Teslimat kaydı bulunamadı!' });
    }
  } catch (err) {
    next(err);
  }
...
app.post('/api/values', async (req, res, next) => {
  try {
    ...
    const [record, created] = await db.StudentValueProgress.findOrCreate({
      ...
      defaults: { progress, badgeName, notes }
    });
    if (!created) {
      record.progress = progress;
      record.badgeName = badgeName || record.badgeName;
...
app.post('/api/finance', async (req, res, next) => {
  try {
    ...
    const [finance, created] = await db.StudentFinance.findOrCreate({
      ...
      defaults: { totalAmount, paidAmount, installments, dueDate }
    });
    if (!created) {
      finance.totalAmount = totalAmount;
      finance.paidAmount = paidAmount;
...
app.post('/api/surveys/vote', async (req, res, next) => {
  try {
    ...
    const resultsMap = JSON.parse(survey.results);
